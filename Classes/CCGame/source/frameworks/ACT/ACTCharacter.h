...
class ACTCharacter : public CCGSprite
{
public:
	...
	ACTCharacter(void);
	virtual ~ACTCharacter(void);

	/** cast spell */
	ACTSpell* castSpell(ACTSpellData* data);
	...
	/** bind action */
	void bindAction(ACTCharacter::ActionState state, std::string animationName) { _actionMap[state] = animationName; }
